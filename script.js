...
async function handleSignUp({ fullName, email, password, phone, matricNumber, department, level, studentIdFile }) {
    ...
    // 2. Upload student ID (may need session; handle gracefully)
    let studentIdUrl = '';
    if (studentIdFile) {
        try {
            ...
            if (!upErr) {
                ...
                studentIdUrl = urlData?.publicUrl || path; // store path; admin generates signed URL
            }
        } catch (_) { /* non-fatal — user can re-upload later */ }
    }

    // 3. Update profile with full registration data
    // NOTE: In email-confirmation mode authData.session is null, so these writes
    // run under the service_role trigger (handle_new_user) context. We use the
    // signed-in session when available; otherwise we rely on the trigger row
    // being in place and just upsert on top of it.
    ...
    if (profileErr) {
        // Non-fatal when email confirmation is required — trigger already created
        // the base row. Log for debugging; do not block sign-up completion.
        console.warn('Profile upsert warning (may be benign in email-confirmation mode):', profileErr.message);
    }

    // 4. Insert verification record — this MUST succeed for admin to review the request
    const { error: verifErr } = await supabase.from('student_verifications').upsert({
        user_id:       userId,
        full_name:     fullName,
        email,
        phone,
    ...
    if (verifErr) {
        // This is critical — without a verification record the admin queue is broken.
        // Throw so the UI shows the user an actionable error rather than silently
        // leaving them with an unverifiable account.
        throw new Error(`Account created but verification record failed to save: ${verifErr.message}. Please contact support.`);
    }
